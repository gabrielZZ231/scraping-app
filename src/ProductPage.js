...
class ProductPage extends BasePage {
  ...
  async getTextByPriority(selectors) {
    for (const selector of selectors) {
      try {
        const val = await this.page.$eval(selector, e => e.textContent);
  ...
  async getAttributeByPriority(selectors, attr) {
    for (const selector of selectors) {
      try {
        const val = await this.page.$eval(selector, (e, a) => e.getAttribute(a), attr);
